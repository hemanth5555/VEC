...
const ImageUploader = (props) => {
  ...
  const handleUpload = () => {
    ...
    formData.append('workflow_name', selectedAgency);
    console.log("Image uploader")
    fetch(baseuri +`/upload_workflow`, {
      method: 'POST',
      headers: {
        "session-token": session_token
      },
      ...
      .then(response => {
        ...
        if (response.ok) {
          setMessage("Upload Successful. Scientific workflow files are sent for execution.");
          setIsVisible(true);
      ...
      <div className="box">
        <div className="box1">
          ...
          <select className="agencyDropdown" value={selectedAgency} onChange={handleAgencyChange}>
            <option value="">Select Workflow Types</option>
            {agencies.map((agency) => (
              <option key={agency.workflow_id} value={agency.workflow_id}>
                {agency.workflow_name}
              </option>
            ))}
          </select>
        </div>
        <div className="box2">
          ...
          <div className="image-uploader-container" onDragOver={handleDragOver} onDrop={handleDrop}>
            <div className="image-upload-box">
              ...
              <input type="file" id="fileUpload" onChange={handleFileChange} accept=".jpg,.jpeg,.png,.pdf,.zip" />
            </div>
            <div className="image-upload-box">
              <label htmlFor="fileUpload">Choose file:</label>
              <input type="file" id="fileUpload" onChange={handleFileChange} accept=".jpg,.jpeg,.png,.pdf,.zip" style={{ display: 'none' }} />
            </div>
